...
<?php $__env->startSection('action-btn'); ?>
    <div class="float-end">
        <?php if($view == 'grid'): ?>
            <a href="<?php echo e(route('projects.list', 'list')); ?><?php echo e(request()->channel == 'activity' ? '?channel=activity' : ''); ?>"
                data-bs-toggle="tooltip" title="<?php echo e(__('List View')); ?>" class="btn btn-sm btn-primary">
                <i class="ti ti-list"></i>
            </a>
        <?php else: ?>
            <a href="<?php echo e(route('projects.index')); ?><?php echo e(request()->channel == 'activity' ? '?channel=activity' : ''); ?>"
                data-bs-toggle="tooltip" title="<?php echo e(__('Grid View')); ?>" class="btn btn-sm btn-primary">
        ...
        <div class="dropdown-menu  dropdown-steady" id="project_sort">
            <a class="dropdown-item active" href="#" data-val="priority-asc">
                <i class="ti ti-sort-ascending"></i><?php echo e(__('Priority')); ?>

            </a>

            <a class="dropdown-item" href="#" data-val="created_at-desc">
        ...
        <a href="#" class="btn btn-sm btn-primary action-item" role="button" data-bs-toggle="dropdown"
            aria-haspopup="true" aria-expanded="false">
            <span class="btn-inner--icon"><?php echo e(__('Department')); ?></span>
        </a>
        <div class="dropdown-menu  project-department-filter-actions dropdown-steady" id="project_department">
            <a class="dropdown-item filter-action filter-show-all pl-4 active" href="#"><?php echo e(__('Show All')); ?></a>
            <?php $__currentLoopData = \App\Models\Department::get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $val): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a class="dropdown-item filter-action pl-4" href="#"
                    data-val="<?php echo e($val->id); ?>"><?php echo e(__($val->name)); ?></a>
        ...
        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('create project')): ?>
            <a href="#" data-size="lg"
                data-url="<?php echo e(route('projects.create')); ?><?php echo e(request()->channel == 'activity' ? '?channel=activity' : ''); ?>"
                data-ajax-popup="true" data-bs-toggle="tooltip"
                title="<?php echo e(request()->channel == 'activity' ? __('Create New Activity') : __('Create New Project')); ?>"
                data-title="<?php echo e(request()->channel == 'activity' ? __('Create Activity') : __('Create Project')); ?>"
...
<?php $__env->startPush('script-page'); ?>
    <script>
        $(document).ready(function() {
            var sort = 'priority-asc';
            var status = '';
            ajaxFilterProjectView('priority-asc');
            $(".project-department-filter-actions").on('click', '.filter-action', function(e) {
                ...
                var filterArray = [];
                var url = $(this).parents('.project-department-filter-actions').attr('data-url');
                $('div.project-department-filter-actions').find('.active').each(function() {
        ...
        function ajaxFilterProjectView(project_sort, keyword = '', status = '') {
            ...
            currentRequest = $.ajax({
                url: '<?php echo e(route('filter.project.view')); ?><?php echo e(request()->channel == 'activity' ? '?channel=activity' : ''); ?>',
                data: data,
                beforeSend: function() {
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Applications/MAMP/htdocs/erp-sakka/resources/views/projects/index.blade.php ENDPATH**/ ?>