...
class ProjectReportController extends Controller
{
        ...
        public function index(Request $request)
    {
        ...
        if($user->type == 'client')
        {
            $projects = Project::where('type', 'sales')->where('client_id', '=', $user->id);
            $users=[];
            $status=[];
        ...
        elseif(\Auth::user()->type == 'company')
        {
            ...
            if(isset($request->all_users)&& !empty($request->all_users)){
                ...
                if(request()->channel == 'activity'){
                    $projects = Project::where('type', 'activity')->where('projects.created_by', '=', $user->id);
                }else{
                    $projects = Project::where('type', 'sales')->where('projects.created_by', '=', $user->id);
                }
            }
        ...
        else
        {
            $users =[];
            if(request()->channel == 'activity'){
                $projects = Project::where('type', 'activity')->select('projects.*')->leftjoin('project_users', 'project_users.project_id', 'projects.id')->where('project_users.user_id', '=', $user->id);
            }else{
                $projects = Project::where('type', 'sales')->select('projects.*')->leftjoin('project_users', 'project_users.project_id', 'projects.id')->where('project_users.user_id', '=', $user->id);
