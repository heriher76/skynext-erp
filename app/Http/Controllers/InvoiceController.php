...
class InvoiceController extends Controller
{
    ...
    public function create($customerId)
    {
        if(\Auth::user()->can('create invoice'))
        {
            ...
            $product_services = ProductService::where('type', 'service')->where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            $product_services->prepend('--', '');
            $projects      = Project::where('type', 'sales')->get()->pluck('project_name', 'id');
            $projects->prepend('Select Project', '');

            return view('invoice.create', compact('projects', 'customers', 'invoice_number', 'product_services', 'category', 'customFields', 'customerId'));
        }
        else
        {
    ...
    public function project(Request $request)
    {
        ...
        return "Budget: Rp " . number_format($project->budget, 2, ",", ".");
    }
...
//        dd($request->all());
        if(\Auth::user()->can('create invoice'))
        {
            ...
            $invoice->category_id    = $request->category_id ?? 2;
...
//                $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function edit($ids)
    {
        if(\Auth::user()->can('edit invoice'))
        {
            ...
            return view('invoice.edit', compact('projects', 'customers', 'product_services', 'invoice', 'invoice_number', 'category', 'customFields'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
...
//                    $invoiceProduct->discount    = isset($products[$i]['discount']) ? $products[$i]['discount'] : 0;
    ...
    public function show($ids)
    {
        ...
        if(\Auth::user()->can('show invoice'))
        {
            ...
            $id      = Crypt::decrypt($ids);
            $invoice = Invoice::with('creditNote')->find($id);
    ...
    public function createPayment(Request $request, $invoice_id)
    {
        ...
        if($invoice->getSubTotal() < $request->amount)
        {
