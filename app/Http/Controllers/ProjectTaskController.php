            ...
            if (Auth::user()->type == 'client') {
                ...
                if (Auth::user()->type == 'company') {
                    if(request()->channel == 'activity'){
                        $data = ProjectTask::join('projects', 'projects.id', '=', 'project_tasks.project_id')->where('type', 'activity')->where('project_tasks.created_by', \Auth::user()->creatorId())->get();
                    }else{
                        $data = ProjectTask::join('projects', 'projects.id', '=', 'project_tasks.project_id')->where('type', 'sales')->where('project_tasks.created_by', \Auth::user()->creatorId())->get();
                    }
                } else {
                    $usr = Auth::user();
                    $user_projects = $usr->projects()->pluck('project_id', 'project_id')->toArray();
