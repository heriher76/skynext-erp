...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ __('Daily Activity') }}</li>
@endsection


@section('action-btn')
    <div class="float-end">
        <a href="{{ route('daily_activity.index') }}" class="btn btn-sm btn-primary" data-bs-toggle="tooltip"
            title="{{ __('List View') }}">
                        ...
                        <div class="row align-items-center">
                            <div class="col-6 text-start">
                                <span data-toggle="tooltip"
                                    data-title="{{ __('Created Date') }}">{{ \Auth::user()->dateFormat($support->created_at) }}</span>
                            </div>
                            <div class="col-6 d-flex float-end">
                                <div class="action-btn bg-warning me-2">
                                    <a href="{{ route('daily_activity.reply', \Crypt::encrypt($support->id)) }}"
                                        data-title="{{ __('Support Reply') }}" class="mx-3 btn btn-sm align-items-center"
                                ...
                                @if (\Auth::user()->id == $support->ticket_created)
                                    <div class="action-btn bg-primary me-2">
                                        <a href="#" data-size="lg"
                                            ...
                                            data-ajax-popup="true" data-title="{{ __('Edit Daily Activity') }}"
