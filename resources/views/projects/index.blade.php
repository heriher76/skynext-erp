...
@section('action-btn')
    <div class="float-end">
        @if ($view == 'grid')
            <a href="{{ route('projects.list', 'list') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                data-bs-toggle="tooltip" title="{{ __('List View') }}" class="btn btn-sm btn-primary">
                <i class="ti ti-list"></i>
            </a>
        @else
            <a href="{{ route('projects.index') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                data-bs-toggle="tooltip" title="{{ __('Grid View') }}" class="btn btn-sm btn-primary">
        ...
        <div class="dropdown-menu  dropdown-steady" id="project_sort">
            <a class="dropdown-item active" href="#" data-val="priority-asc">
                <i class="ti ti-sort-ascending"></i>{{ __('Priority') }}
            </a>
        ...
        <a href="#" class="btn btn-sm btn-primary action-item" role="button" data-bs-toggle="dropdown"
            aria-haspopup="true" aria-expanded="false">
            <span class="btn-inner--icon">{{ __('Department') }}</span>
        </a>
        <div class="dropdown-menu  project-department-filter-actions dropdown-steady" id="project_department">
            ...
            @foreach (\App\Models\Department::get() as $key => $val)
                <a class="dropdown-item filter-action pl-4" href="#"
                    data-val="{{ $val->id }}">{{ __($val->name) }}</a>
            @endforeach
        </div>
        ...
        @can('create project')
            <a href="#" data-size="lg"
                data-url="{{ route('projects.create') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                data-ajax-popup="true" data-bs-toggle="tooltip"
                title="{{ request()->channel == 'activity' ? __('Create New Activity') : __('Create New Project') }}"
                data-title="{{ request()->channel == 'activity' ? __('Create Activity') : __('Create Project') }}"
...
@push('script-page')
    <script>
        ...
        function ajaxFilterProjectView(project_sort, keyword = '', status = '') {
            ...
            currentRequest = $.ajax({
                url: '{{ route('filter.project.view') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}',
                data: data,
                beforeSend: function() {
