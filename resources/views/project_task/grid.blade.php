    ...
    <div class="float-end">
        ...
        @else
            <a href="{{ route('taskBoard.view', 'grid') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="{{ __('Card View') }}">
                                ...
                                <div class="card m-3 card-progress border shadow-none" id="{{ $task->id }}"
                                    ...
                                    <div class="card-body">
                                        <div class="row align-items-center mb-2">
                                            <div class="col-6">
                                                <span
                                                    class="badge p-2 px-3 rounded bg-{{ \App\Models\ProjectTask::$priority_color[$task->priority] }}">{{ \App\Models\ProjectTask::$priority[$task->priority] }}</span>
                                            </div>
                                        ...
                                        <a class="h6 task-name-break"
                                            href="{{ route('projects.tasks.index', !empty($task->project) ? $task->project->id : '') }}">{{ $task->name }}</a>
                                        <div class="row align-items-center">
                                            ...
                                            <div class="col-6">
                                                @if (!empty($task->end_date) && $task->end_date != '0000-00-00')
                                                    <small
                                                        @if (strtotime($task->end_date) < time()) class="text-danger" @endif>{{ Utility::getDateFormated($task->end_date) }}</small>
                                                @endif
                                            </div>
                                            <div class="col-6 text-end">
                                                @if ($users = $task->users())
