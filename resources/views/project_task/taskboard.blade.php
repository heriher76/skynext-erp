...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><a
            href="{{ route('projects.index') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}">{{ __('Project') }}</a>
    </li>
    ...
    <div class="float-end">
        ...
        <div class="dropdown-menu dropdown-menu-right task-filter-actions dropdown-steady" id="task_status">
            ...
            <a class="dropdown-item filter-action pl-4 active" href="#"
                data-val="see_my_tasks">{{ __('See My Tasks') }}</a>
            <hr class="my-0">
            ...
            <a class="dropdown-item filter-action filter-other pl-4" href="#"
                data-val="due_today">{{ __('Due Today') }}</a>
            <a class="dropdown-item filter-action filter-other pl-4" href="#"
                data-val="over_due">{{ __('Over Due') }}</a>
            <a class="dropdown-item filter-action filter-other pl-4" href="#"
                data-val="starred">{{ __('Starred') }}</a>
        </div>

        @if ($view == 'grid')
            <a href="{{ route('taskBoard.view', 'list') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="{{ __('List View') }}">
                <span class="btn-inner--text"><i class="ti ti-list"></i>{{ __('List View') }}</span>
            </a>
        @else
            <a href="{{ route('taskBoard.view', 'grid') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}"
                class="btn btn-primary btn-sm" data-bs-toggle="tooltip" title="{{ __('Grid View') }}">
...
@push('script-page')
    <script>
        ...
        function ajaxFilterTaskView(task_sort, keyword = '', status = '') {
            ...
            $.ajax({
                url: '{{ route('project.taskboard.view') }}{{ request()->channel == 'activity' ? '?channel=activity' : '' }}',
                data: data,
